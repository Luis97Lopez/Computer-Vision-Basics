function slitScanHorizontal()
  ...
  % and lines per scan
  bufferSize          = round(vidWidth/nLinesPerScan);
  circBuff            = videoIn(1:bufferSize);
  
  % Calculate vertical position
  leftIdx              = 1:nLinesPerScan:vidWidth-nLinesPerScan;
  rightIdx             = leftIdx + nLinesPerScan;
  ...
  for lastFrame       = 1:nFrames;
    ...
    for iBuffer     = 1:thisBuffer,
      videoOut(lastFrame).frame(:,leftIdx(iBuffer):rightIdx(iBuffer),:)...
          = circBuff(iBuffer).frame(:,leftIdx(iBuffer):rightIdx(iBuffer),:);
    end
